...
class RegionsSelector(SelectorModel):
    ...
    @classmethod
    def from_json(cls, regions_mask, wcs_info, wcs_regions=None, dist_info=None, spec_info=None):
        """
        ...
        regions_mask : ndarray
            ...
            ``undefined_transform_value`` if provided.
        wcs_info : dict like
            Basic WCS keywords for reference region
        wcs_regions : dict like
            Basic WCS keywords for each region, expressed as
            deltas from the reference region
        dist_info : dict like
            distortion models for each region
        spec_info : dict like
            spectral models for each region
        ...
        sky_wcs, spec_wcs = None, None
        for rid in labels:
            if wcs_regions is not None:
                for reg in wcs_regions['regions']:
                    if reg['id'] == rid:
                        rid_wcs = reg
                wcs_info = self.reconstruct_wcs(rid_wcs, wcs_info)
            if dist_info is not None:
                #if isinstance(dist_info, dict):
                if len(dist_info['regions']) > 1:
                    for reg in dist_info['regions']:
                        if reg['id'] == rid:
                            sky_wcs = transform_from_json(reg)
                else:
                    sky_wcs = transform_from_json(dist_info['regions'][0])
            else:
                rid_dist_info = None
            for reg in spec_info['regions']:
                if reg['id'] == rid:
                    spec_wcs = transform_from_json(reg)
            transform = CompositeSpectralWCS(sky_wcs, spec_wcs)
            transforms.append(transform)
        return RegionsSelector(regions_mask, labels, transforms)
